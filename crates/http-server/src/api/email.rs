...
use db::models::temp_address::{TempEmailRequest, TempEmailResponse};
use db::services::email::{
    delete_all_emails_by_address, delete_email_by_id, get_email_detail_by_address,
    list_email_summaries_by_address,
};
use db::services::temp_address::{create_temporary_email, find_by_address};
...
pub async fn delete_email_by_id_handler(
    ...
    let deleted_item = delete_email_by_id(&app_state.db_pool, &address, email_id)
        .await
        .map_err(db_error_to_api)?;
